          ...
          <div className="space-y-2">
            ...
            {items.map((item, index) => (
              <div key={item.id} className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end md:items-center bg-gray-50 p-4 md:p-2 rounded-xl border border-gray-200 relative">
                {/* Mobile Label */}
                <div className="md:hidden absolute top-2 left-4 text-xs font-bold text-black">{index + 1}. {item.name} ({item.hindiName})</div>
                ...
                <div className="grid grid-cols-2 md:col-span-4 gap-3">
                  <div>
                    <label className="md:hidden text-xs text-black mb-1 block">Qty</label>
                    <input
                      type="number"
                      placeholder="Qty"
                      value={item.quantity || ""}
                      onChange={(e) => handleItemChange(item.id, "quantity", parseFloat(e.target.value) || 0)}
                      className="w-full p-2 border border-gray-400 rounded-md focus:ring-2 focus:ring-green-500 outline-none text-sm text-black"
                    />
                  </div>
                  <div>
                    <label className="md:hidden text-xs text-black mb-1 block">Rate</label>
                    <input
                      type="number"
                      placeholder="Rate"
                      value={item.rate || ""}
                ...
                <div className="md:col-span-3 text-right font-bold text-black text-sm">
